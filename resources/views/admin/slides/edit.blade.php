...
@section('title', 'Админка слайд | edit')
		...
		<div class="card radius-15">
			<div class="card-body">
				<div class="page-breadcrumb d-none d-md-flex align-items-center mb-0">
					<div class="breadcrumb-title pr-3">Редактировать слайд </div>
					<div class="pl-3">
...
<div class="card radius-15">
	<div class="card-body">
		...
		<form action="{{ route('admin.slide.update', $slide->id) }}" method="post" autocomplete="off" enctype="multipart/form-data">
			@csrf
			...
			<div class=" mb-3">
				...
				<select class="custom-select @error('product_id') error_line @enderror" name="product_id" id="product_id">
					<option selected disabled>Выберите номер отеля, к которой будет прикреплено фото</option>
					@foreach ($products as $product)
					<option
						@if($product->id == $slide->product_id) selected @endif
			...
			<div class="mb-3">
				...
				<input class="form-control @error('title') error_line @enderror" id="title" name="title" type="text" placeholder="" value="{{ $slide->title }}">
				@error('title')
					<p class="mistake">{{ $message }}</p>
				@enderror
			</div>
			...
			<div class="mt-4">
				<img id="showPic" style="width: 500px; margin-bottom:30px;" src="{{ asset( $slide->slide ??'uploads/not_image.jpg') }}" alt="">
			</div>
